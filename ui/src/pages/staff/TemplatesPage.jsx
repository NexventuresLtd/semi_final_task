...
export default function TemplatesPage() {
  ...
  return (
    <>
      <Helmet><title>{t("templates.title")} — FERWAFA Approvals</title></Helmet>

      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h1 className="font-display text-2xl font-semibold text-ink dark:text-ink-dark">{t("templates.title")}</h1>
        <div className="flex gap-1.5 p-1 rounded-lg bg-surface-light dark:bg-glass-dark">
          ...
          >
            <LayoutList className="w-3.5 h-3.5" /> {t("templates.myTemplates")}
          </button>
          <button
            onClick={() => setTab("add")}
          ...
          >
            <Plus className="w-3.5 h-3.5" /> {t("templates.addTemplate")}
          </button>
        </div>
      </div>
          ...
          <motion.div key="view" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
            {isLoading ? (
              ...
              <GlassCard className="text-center py-10">
                <p className="text-ink-muted dark:text-ink-muted-dark mb-4">{t("templates.noTemplatesSaved")}</p>
              </GlassCard>
            )}
          </motion.div>
        ) : (
