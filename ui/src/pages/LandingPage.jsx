...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-slate-900 text-white transition-colors duration-300">
      {/* ================= BACKGROUND GEOMETRY (MATCHES AUTH PAGE) ================= */}
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
        {/* Base Gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#0c2856] via-[#091b3a] to-[#040d1c]" />

        {/* Diagonal Ribbons (Yellow & Blue Layers) */}
        <div className="absolute -top-24 -right-24 w-[700px] h-[1000px] pointer-events-none opacity-90">
      ...
      <header className="relative z-10 flex items-center justify-between px-6 lg:px-16 py-6 max-w-[1400px] mx-auto">
        <div className="flex items-center gap-3">
          ...
          <span className="font-display text-white font-semibold text-lg tracking-tight">
            FERWAFA · <span className="text-emerald-400">{t("landing.departmentsTag")}</span>
          </span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur-md border border-white/10 transition-all cursor-pointer"
            aria-label="Toggle Theme"
          >
            {isDark ? <Sun className="w-4 h-4 text-yellow-300" /> : <Moon className="w-4 h-4 text-slate-200" />}
          </button>
      ...
      <main className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-16">
        ...
        <section className="grid lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center pt-10 pb-20 lg:pt-16 lg:pb-28">
          ...
          >
            <p className="text-xs font-medium tracking-[0.15em] uppercase text-blue-300 mb-4 flex items-center gap-2">
              <span className="inline-block w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              {t("landing.eyebrowTag")}
            </p>
            <h1 className="font-display text-white text-4xl sm:text-5xl lg:text-[3.4rem] font-bold leading-[1.08] max-w-xl">
              {t("landing.heroTitlePart1")}
              <br />
              <span className="text-blue-200">{t("landing.heroTitlePart2")}</span>
            </h1>
            <p className="text-slate-300 max-w-md mt-5 text-[15px] leading-relaxed">
            ...
            <div className="flex flex-wrap gap-3 mt-8 items-center">
              ...
              <Link to="/create-account">
                <button
                  className="text-sm cursor-pointer text-blue-200 border border-blue-400/30 bg-white/5 backdrop-blur-md rounded-md p-[13px] hover:bg-white/10 transition-all"
                >
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-3 mt-12 pt-8 border-t border-white/10">
              <div className="flex items-center gap-2 text-sm text-slate-300">
                <FileCheck2 className="w-4 h-4 text-emerald-400" /> {t("landing.threeStage")}
              </div>
              <div className="flex items-center gap-2 text-sm text-slate-300">
                <Bell className="w-4 h-4 text-emerald-400" /> {t("landing.notifiedEveryStep")}
              </div>
              <div className="flex items-center gap-2 text-sm text-slate-300">
                <Lock className="w-4 h-4 text-emerald-400" /> {t("landing.invitationOnly")}
              </div>
            </div>
          </motion.div>
        ...
        <section className="py-16 border-t border-white/10">
          <h2 className="font-display text-2xl text-white font-semibold mb-10 max-w-md">
            {t("landing.processTitle")}
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            {PROCESS.map((step) => (
              <div key={step.n} className="relative bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-xl">
                <span className="text-amber-400 font-mono font-bold text-sm block mb-2">{step.n}</span>
      ...
      <footer className="relative z-10 border-t border-white/10 mt-8 bg-black/20 backdrop-blur-md">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-16 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <Link to="/login" className="text-xs text-blue-300 hover:text-blue-200 hover:underline transition-colors">
